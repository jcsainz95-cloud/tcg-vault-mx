...
- **Follow-up CI (devops, no urge):** `check-format-mix.sh` (BL-27) da FALSO POSITIVO cuando una edición de lógica ACORTA una línea sobre-ancha (deja el fichero prettier-limpio como efecto colateral). Medido en #50 (`refs-raw-emulate.ts`). Arreglo: antes de declarar mezcla, comprobar que el reformateo de prettier(BASE) cae en líneas que el commit NO tocó. Es norma del arquitecto ⇒ validar con arquitecto/techlead + añadir caso al canario. NO bloquea merges (rojo informativo, no required).
 D1 N+1 pre-existente en checkout `orders.service.ts:389 buildLines` (batchear, backend); D2 drift documental de P-53 (TECH_DEBT BE-80 apunta a código retirado `MANUAL_REF_PREDICATE`/`SAME_DAY_REF_CANDIDATES` → cerrar; ARCHITECTURE describe diseño capado viejo → arquitecto sync); filas §0-Q de EQ-D1 (arquitecto, ya candadas PENDIENTE-ARQUITECTO); nota postgres superusuario local password (`qapw53`, futuros agentes usan rol propio).
...
- **P-53 cura de disco:** ⏸️ **APARCADO — espera decisión A/B del dueño (2026-09-18).** Reveló ser un cambio SISTÉMICO, no una cura contenida: el write-on-change congela `capturedDate` de la fila vigente, y **múltiples lectores de dinero rankean la frescura por `capturedDate`** ⇒ cada arreglo destapó otro sitio acoplado. Historial:
  - `be-p53-cura` (`ad26e1ab`): RECHAZADO seguridad — ALTO-1 (escritor equivocado: tocó `upsertVariantPrice`=import, no el diario `persistMarketReference`) + ALTO-2 (`hasRecentIngest` sobre evidenceDate que el diario no escribía ⇒ re-dispara barrido cada boot).
  - `be-p53-cura2` (`0e76a37f`): QA ✅ + techlead ✅; RECHAZADO seguridad — ALTO-3 (la selección en LOTE `getReferencesBatch`/`isBetterRef`/`computeSetValue` rankeaba por capturedDate ⇒ fallback fresco tapa la primaria buena congelada).
  - `be-p53-cura3` (`b9c856bd`): arregló ALTO-3 (COALESCE(evidenceDate,capturedDate) en los 3 sitios). QA ✅; RECHAZADO techlead+seguridad — **ALTO-4** (mismo defecto en `getReference`/`getReferenceByCardProduct`, ventana `take:32` por capturedDate crudo — ruta money MÁS usada: órdenes, buylist, vault). Verdictos seguridad en tags `seg-p53v2-verdict-0e76a37f`, `seg-p53v3-verdict-b9c856bd` (no empujados).
  - **Decisión pendiente del dueño:** (A) barrido de diseño del arquitecto que audite TODOS los lectores de `capturedDate`-como-frescura y los arregle de una vez → 1 implementación → 3 gates; o (B) aparcar P-53 (disco crece despacio, no urge) y quedarse con las victorias de la noche. **Corrige también:** `BACKEND_NOTES §0.1.b` afirma que la cota `take:32` es money-safe para automáticas — hoy FALSO bajo write-on-change. NADA de P-53 sale sin visto bueno explícito del dueño (money).
...
- **Gates (todos sobre `79591fe7`):** QA **APROBADO** (idéntico byte-a-byte con/sin índice; unit 5344+1883, integr. 329, migr. 6/6, paridad 43/43) · techlead **APROBADO** con deuda · arquitecto **RATIFICADO** (aditivo, money-safe) · orquestador **O-9** verificado por mí sobre copia del árbol entero (candado money muerde: mutación de escritura → 2 rojas; es/en simétricos 3058/3058).
- **Condición del techlead pendiente (dueño):** correr la migración en ventana de bajo tráfico (está en el cuerpo del PR, donde está el botón).
- ⚠️ **INCIDENTE (LOCAL, no prod):** un agente destruyó sin querer la evidencia forense `SPEI-EJE2-REMEDIADA` (y marcas `SPEI-*/QA-BL35/PENTEST-/POC-/REDTEAM-`) en la **BD de PRUEBAS nativa** al correr un spec con `seedE2E` (patrón `-t` mal citado). **Producción intacta** (egress bloqueado). Dueño: **pentester/seguridad** recrean la evidencia (recuperable solo de un snapshot de la BD nativa). No bloquea publicación.

## 🐢 PERF-CATÁLOGO — la causa REAL de los ~20s (medido por el dueño, Network tab, 2026-09-17)
**NO son las imágenes** (cargan ~200ms, 11-45kB avif). El cuello son **3 fetch del catálogo, ~10s c/u**, medidos por HTTP en prod: `GET /catalog/cards?sort=grading_showcase&pageSize=8` (10.16s), `?sort=price_desc` (9.55s), `GET /catalog/facets` (9.50s). pageSize=8 y aún 10s ⇒ **consulta lenta, no volumen**. Home y comprar/vender tardan porque todas pegan a esos endpoints.
- **CAUSA 1 — índice:** PUBLICADO (#41, production `4a3caa64`).
- **CAUSA 2 — histórico de precios (H-PERF-1, la que DOMINABA ~96%):** arreglada. `claude/perf-catalog-2` (`03ab987e`) poda el histórico en la BD con `$queryRaw` (ventana `max_auto_date` ∪ manuales; DISTINCT), money-safe (salida byte-idéntica, probado por QA). **3 veredictos verdes** (QA/techlead/seguridad sobre `12927edd`) + O-9 mío (código idéntico al gateado; canario de poda muerde; el oráculo e2e de dinero lo corrió QA, yo no tengo credencial BD local). NO toca schema/migración. **PR #42 esperando el click:** https://github.com/jcsainz95-cloud/tcg-vault-mx/pull/42
- **CAUSA 3 — config por carta:** resultó ya arreglada (~0,1%, se lee una vez por request). No había nada que hacer.
- **Ganancia medida (local 3.000×60):** listCards ~2,9s→~0,47s (~6×), facets ~6,7×. El nº exacto en prod lo mide el dueño (O-17, egress bloqueado aquí).
- **Deuda anotada (arquitecto, regla 9):** BE-89 índice de cobertura de `PriceReference` (baja el suelo ~170ms); BE-90 H-PERF-3 materializar `displayPrice`. La rama `claude/perf-imagenes` (preconnect) queda como mejora menor, NO era la cura.
...
### ✅ M11-UX — RECONSTRUIDO, listo para publicar (2026-09-17)
`claude/fe-m11-ux` (`4a73d707`, solo frontend, base production). 3 capas (inventario arriba · botón único «Actualizar precios de la colección» · «Ajustes avanzados» plegado) + «Arreglar este set» condicional + modal re-rotulado en llano. **Motor/endpoints/permisos INTACTOS** (SuperAdminOnly sigue puesto). Verificado por el orquestador (O-9): tsc limpio, M11 vitest 32/32, candado de rol MUERDE. Gate proporcional: es SOLO presentación (cero lógica/endpoint/permiso nuevos) ⇒ verificación del orquestador, sin re-correr los 3 filtros. Compare `production...claude/fe-m11-ux`. Deuda menor de M11 (SEC-M11-3/4/5, `claude/m11-debt`) sigue aparte, pendiente de sus gates.

### 🎨 M11-UX — REDISEÑO por caos de UX (dueño en tienda, 2026-09-17)
M11 se publicó y **funciona**, pero el dueño la halló **caótica**: «es un apartado para subir/bajar producto y editar precio/despublicar lo publicado; actualizar precios debería ser MUY sencillo; el "estado por set" con su pop-up no se entiende; hay dos apartados de diales y no sé qué hace cada uno. Si yo que la construí me pierdo, imagina al delegar.» Es **presentación/IA, no el motor**. Norte del rediseño: (1) inventario arriba (alta/editar precio/despublicar); (2) UN botón «Actualizar precios de la colección» que haga todo y hable llano; (3) lo técnico (mapeo TCGCSV, proveedor/tendencia/alertas, spreads) PLEGADO en «Ajustes avanzados», y el arreglo de un set sin precio solo cuando haga falta. **En diseño:** ux-ui `claude/ux-m11-redisenio` → luego frontend reconstruye la presentación (sin tocar backend/precios). Aprende: entregar «funcional» no basta si el usuario se pierde (O-4 en clave UX).
