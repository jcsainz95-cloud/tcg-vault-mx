...
| **S3-ENVIO-DIR** | 🔴 **La pantalla de envíos del operador (M4) NO muestra la calle ni el número** (medido en el envío del pedido TCG-000012). Pinta nombre · ciudad · estado · CP · teléfono, pero **omite `line1`/`line2`/`neighborhood`** (calle, número exterior/interior, colonia): `M4View.tsx:284-294` no los renderiza. **El dato SÍ está guardado** (`AddressSnapshotDTO.line1` es obligatorio; el checkout de invitado lo exige y captura). El operador **no puede enviar** porque no ve a qué calle mandar. **Solo pantalla ⇒ arreglo de frontend**, sin tocar contrato ni backend. Bloqueante para operar (no toca dinero; sin ventas reales aún). | frontend | 2026-09-15 | `frontend/src/app/[locale]/(admin)/admin/m4/M4View.tsx:284-294`; `frontend/src/types/contract.ts:1155-1161` (`AddressSnapshotDTO` con `line1/line2/neighborhood`) |
