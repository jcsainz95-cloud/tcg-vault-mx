...
| **P-89** | ⚠️ **El censo que declaró conforme al catálogo se equivoca justo en el borde que P-84 acaba de descubrir** (techlead, 2026-09-13; **re-verificado por el orquestador** leyendo los ficheros). Backend paró la consolidación de H3 sin migrar las dos copias del catálogo, y citó como motivo que «§0-Q punto 4 las declara ya conformes». **El censo declaró conforme mirando la FORMA del `details`, no el borde del espacio en blanco** — el mismo error que ese censo cometió con `/admin/users`, que backend sí cazó y cerró. Medido: `catalog.service.ts:1129-1133` y `sealed-catalog.service.ts:240-243` siguen con `if (q.X)` + `as never`, así que `?condition=%20` es truthy ⇒ `400`, donde §0-Q punto 1 fila 1 exige **`200` sin filtrar**. **MEDIDO POR HTTP el 2026-09-13** — primero por QA en su veredicto de P-84, y **reproducido por el orquestador** contra el stack vivo (`bd1f951`, endpoint **público, sin token**): `/catalog/cards?condition=%20`⇒**400**, `?productType=%20`⇒**400**, `?finish=%20`⇒**400**, `?sealedSubtype=%20`⇒**400**; `/catalog/sealed?sealedSubtype=%20`⇒**400**; `/catalog/sealed?productType=%20`⇒**200** (ésta sí conforme). **Cinco ejes de seis**, y en endpoint público — exposición **mayor** que la admin de P-84. El «NO MEDIDO» de esta ficha queda **cerrado**. Severidad **baja** — es `400`, no `500`; no toca dinero; el cliente tipado no lo alcanza (`frontend/src/lib/api.ts:293-295`). **Lo grave no es el `400`: es la nota.** Una ficha que dice «conformes» garantiza que nadie vuelva a mirar (O-5). Migrar además **borra cuatro `as never`**, porque el helper único es genérico. Cierra: un test por HTTP de `?condition=%20` ⇒ `200`, y la ficha H3 corregida. Es del stream **Catálogo y precios**, no del de P-84. | backend (H3 + migración) · arquitecto (refrescar el censo de §0-Q punto 4, `API_CONTRACT.md:4981-4996`) | **2026-09-13 (ejecutado por HTTP: QA + orquestador, 5/6 no conformes)** | `catalog.service.ts:1129-1133`; `sealed-catalog.service.ts:240-243`; `docs/TECH_DEBT.md:613-620` |
