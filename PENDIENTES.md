...
- **(D) Sellado SIN PRECIO DE MERCADO — NO es falta de mapeo.** ✅ Medido: `SealedProduct.tcgplayerProductId` es `Int @unique` **NOT NULL** (schema). El «Chaos Rising» no tiene precio porque **no hay `PriceReference` resuelta** bajo `sealed:tcg:<id>` — es **P-46** (Chaos Rising sin grupo TCGCSV) **+ P-83**. Money-path ⇒ **arquitecto + 3 veredictos**. No tocado.
