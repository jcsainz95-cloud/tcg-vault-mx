...
**NO son las imágenes** (cargan ~200ms, 11-45kB avif). El cuello son **3 fetch del catálogo, ~10s c/u**, medidos por HTTP en prod: `GET /catalog/cards?sort=grading_showcase&pageSize=8` (10.16s), `?sort=price_desc` (9.55s), `GET /catalog/facets` (9.50s). pageSize=8 y aún 10s ⇒ **consulta lenta, no volumen** (índice faltante / agregación de facets / enriquecimiento por-carta). Home y comprar/vender tardan porque todas pegan a esos endpoints. **CAUSA 1 arreglada y EN PR #41** (índice aditivo, ver arriba). **Faltan causas 2 y 3** (enrutadas al arquitecto, NO en #41): H-PERF-1 (lectura de histórico de precios, `pricing.service.ts:877`/`:992`), H-PERF-2 (viajes de config al pintar cada carta), H-PERF-3 (materializar `displayPrice`). **El índice NO baja los 20s completos** (techlead lo subrayó). La rama `claude/perf-imagenes` (preconnect) queda como mejora menor, NO era la cura.
