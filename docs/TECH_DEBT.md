...
- **(a) ⚠️ «Los dos comparadores divergen con `label: ''`» — NO SE REPRODUCE, y la medición dice lo contrario.** La nota, tal como llegó, decía que el backend ordena **primero** una etiqueta vacía y el cliente **última**. Medido hoy (2026-09-22) sobre `backend/src/modules/shipments/shipments.service.ts`:
  - el comparador **no está en `:871-877`** (ahí vive el mapper del `addressSnapshot`) sino en **`byLocation`, `:969-976`**;
  - y, sobre todo, **el backend nunca emite `label: ''`**: `locationViewOf` (`:958-961`) normaliza con `nullIfBlank` (`:160-163`) y una etiqueta en blanco **omite la llave**, así que llega como `undefined` ⇒ su propio comparador la manda **al final** (`la === undefined ⇒ return 1`).
...
- **La deuda:** §30.12 (apilado en orden FIJO MANUAL→AUTOMÁTICA, cifra a 22px, interruptor a ancho completo, diálogos como *bottom sheet*, campo ≥16px) y la mitad visual de §30.13 (que la tarjeta **siga diciendo lo mismo** sin CSS) están **implementadas con las clases del sistema** y cubiertas por **29 tests en jsdom** — pero **jsdom no aplica CSS**. El smoke E2E nuevo (`e2e/admin-fx.spec.ts`) corre a **1280×800** y mide **conducta**, no layout.
- **Precedente exacto, y por eso se anota:** es la misma familia del hallazgo de §28.9 (la consola de bounties desbordaba **334 px** a 390 y `PAGAMOS` quedaba **fuera de la pantalla**, en una superficie de dinero, sin que ningún test pudiera cazarlo). Aquí el riesgo es menor —hay menos tabla— pero **el mecanismo de ceguera es idéntico**.
- **Cura:** un `test.describe` a **390×844** en `e2e/admin-fx.spec.ts` con lo que sólo existe cuando hay layout: cero desbordamiento horizontal, las dos columnas apiladas **en el orden fijo**, `ack.whereFrom` **completo y sin recortar**, y el campo de la tasa manual a **≥16px**.
- **⚠️ Disparador:** **el primer cambio de layout de la tarjeta** (o que alguien pida usarla desde el teléfono). Ref: `DESIGN_SYSTEM.md` §30.12/§30.13, `FRONTEND_NOTES.md` §61.5.4.

### FX-F3 · Guardar una tasa manual con `source: "fallback"` **no dice nada** — falta la variante de copy (frontend, bloqueada en ux-ui, 2026-09-09)
- **Dueño:** **frontend** (`FxRateCard.tsx`, `savedNotice`), **con la decisión de copy en ux-ui**. **Severidad:** Baja. **No bloqueante.**
- **La deuda:** el acuse de §30.9c tiene dos frases —`manual.savedRuling` (el número rige) y `manual.savedNotRuling` (no rige)— y la segunda dice *«seguimos en automática (**{rate}** de Banxico)»*. En el estado `fallback` **no hay ninguna tasa de Banxico que interpolar**, así que la línea **no se pinta**: se da la mitad visual (la columna MANUAL se repinta con el número nuevo, **sin** `RIGE`) y **se calla el texto**.
- **Por qué se calla en vez de resolverlo aquí:** las dos salidas caseras eran **inventar la frase** (⛔ §30.15: el copy se copia sin interpretar) o **meter el valor de respaldo donde el copy dice «de Banxico»** (⛔ y ésta es peor: *nunca se sustituye un número por otro que significa otra cosa* — el compromiso que este mismo panel adquirió al cerrar B-1/B-2).
- **Impacto:** en ese estado concreto, guardar una tasa **puede parecer que no hizo nada** para quien no mire la columna. Es una versión atenuada de `Q-F3`, el defecto que `savedNotRuling` vino a cerrar.
- **Cura:** **una cadena ES/EN de ux-ui** para «guardada; no rige, y no hay tasa de Banxico»; cablearla es una línea. El hueco lo levantó ya `FRONTEND_NOTES §60.6` y **sigue abierto**.
- **⚠️ Disparador:** **que ux-ui entregue la variante** — o, antes, que alguien reporte «guardé y no pasó nada» en un entorno sin fila de Banxico (que es **el estado real de producción hoy**, `D-OPS-1`). Ref: `DESIGN_SYSTEM.md` §30.9c/§30.15, `FRONTEND_NOTES.md` §60.6 y §61.5.1.
...
### EQ-D0 · ✅ **CERRADA (2026-09-13)** — La BÓVEDA ignoraba en silencio los filtros DEL CLIENTE (backend · Inventario y vault)

> ⭐ **CERRADA el 2026-09-13, medido.** `vault.service.ts` pasa los tres ejes por `parseEnumFilter`
> (el helper único de §0-Q) y ordena con un `switch` **exhaustivo** con `never` en el `default` —el
> `else` que clampaba en silencio ya no existe—. Pruebas:
> `backend/test/integration/vault-sealed-enum-filters.e2e-spec.ts` (**27/27**, las DOS rutas con la
> sesión de cada rol) y `backend/test/vault-sealed.spec.ts` (16/16). Los **seis** ejes están ahora en
> el `REGISTRO` de `C-EQ-1` con sus siete propiedades verdes **sin excepción**, y el trinquete bajó
> de **22 a 16** (la ficha preveía 18: se cerraron **seis** ejes, no cuatro — el `?sort=` de las dos
> rutas entró también, porque el clamp silencioso lo prohíbe §0-Q punto 6 con el mismo argumento).
> Mutaciones sobre copia del árbol entero: `M-V1` (vuelve el `if (x && SET.has(x))`) **3/3 roja**;
> `M-V2` (vuelve el clamp del orden) **3/3 roja**; `M-V3b` (el literal del orden se separa de la
> línea del contrato) **3/3 roja**.
>
> ⚠️ **Lo que NO cierra esta ficha, y es del arquitecto (regla 9):** la **fila de §0-Q punto 4** para
> estos seis ejes sigue sin existir. La conducta conforma; el registro del contrato no los nombra. En
> `C-EQ-1` van marcados `filaEn0Q: 'PENDIENTE-ARQUITECTO'`, con un test que fija exactamente cuáles
> son para que ni se olviden ni crezcan en silencio.
>
> ⚠️ **Hallazgo del cierre, y es la lección:** había un test unitario **verde** que AFIRMABA el
> defecto — `vault-sealed.spec.ts`, *«un subtipo INEXISTENTE se sigue ignorando (tolerar basura
> desconocida SÍ es correcto)»*, que exigía `where.sealedSubtype === undefined`. Venía de `v2.1.8`,
> que arreglaba otra cosa y **de paso dejó escrita como decisión la mitad que no había mirado**. *Un
> defecto con prueba verde deja de parecer un defecto: parece una decisión.* Reescrito, con el
> historial dentro.
...
- **Ejes afectados (4 al abrirla; ⭐ **6** al cerrarla):** `?sealedSubtype=` y `?condition=` en
  `GET /vault/sealed` **y** en `GET /admin/vaults/:userId/sealed`. Al medirlo se sumó el `?sort=` de
  **las dos rutas**: el encargo pedía «mídelo también y di qué hace hoy», y lo que hacía era **clamp
  silencioso** (`?sort=zzz` ⇒ `200` ordenado por valor, sin decirlo) — §0-Q punto 6 lo prohíbe con el
  mismo argumento que el punto 1 prohíbe ignorar el filtro, y tampoco depende de la clase. Su dominio
  **no se inventó**: lo declara la línea del endpoint en `API_CONTRACT.md` §3 (*«`sort` default
  `value_desc`; también `count_desc | name_asc`»*), y la paridad contrato↔literal la vigila `C-EQ-1`.
- **Comprobación de cierre:** ✅ los **seis** pasaron de `SIN_CLASE_DECLARADA` al `REGISTRO` de
  `backend/test/integration/enum-query-axes.e2e-spec.ts`, sus siete propiedades de §0-Q salen verdes
  **sin excepción**, y el trinquete de esa suite bajó de **22 a 16**.
...
—`SIN_CLASE_DECLARADA.length ≤ **16**` (bajó de 22 al cerrarse `EQ-D0`; ver esa ficha) y
`QUERY_SIN_NOMBRE.length ≤ 2`, **números que solo bajan**— y esta
...
- **Comprobación de cierre:** `?productType=` de `/catalog/cards` pasa a `ProductType` **menos**
  ...
  excepción `error` como la entrada de `SIN_CLASE_DECLARADA` (el trinquete baja de 16 a 15 — era 22
  cuando se escribió esta línea, y `EQ-D0` lo bajó a 16).
