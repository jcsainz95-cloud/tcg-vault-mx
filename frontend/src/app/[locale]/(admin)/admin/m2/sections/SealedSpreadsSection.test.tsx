...
describe('SealedSpreadsSection · una fila por valor del enum (T-1)', () => {
  ...
  it('escribir en UPC lo manda al PUT (el dueño YA puede calibrar lo que vende)', async () => {
    const put = vi
    ...
    expect(put).toHaveBeenCalledWith({
      spreadPctBySubtype: { ...PARTIAL_SPREADS.spreadPctBySubtype, upc: 20 },
      fallbackPct: 25,
    });
  });

  it('money-safe: una fila vacía NO se guarda como 0% (vender al costo)', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Guardar' }));
    // El borrador vacío se ignora: la llave conserva su 18, JAMÁS se manda 0.
    expect(put).toHaveBeenCalledWith({
      spreadPctBySubtype: PARTIAL_SPREADS.spreadPctBySubtype,
      fallbackPct: 25,
    });
  });
