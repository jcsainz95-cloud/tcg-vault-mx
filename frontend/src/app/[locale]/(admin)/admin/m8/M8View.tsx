...
export function M8View() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      >
        {/* P-97: sin rama de vacío, cero disputas pintaba el título y NADA más — una pantalla
            sana leída como rota. Se usa el `EmptyState` compartido (DESIGN_SYSTEM §8.1), el mismo
            de §M3/§M4/§M5, no un mensaje escrito a mano aquí.
            ⚠️ UN SOLO copy, a diferencia de §M4: esta cola **no tiene filtro**, así que no existe
            el caso «no hay nada que case con lo que pediste» — solo «no hay nada», que además es
            buena noticia (`tone="positive"`, §8.1 «cola admin vacía»). El día que se añada un eje
            de estado (el contrato ya expone `?status=`, API_CONTRACT §M8) harán falta los DOS
            copys, como en §M4. */}
        {query.data && query.data.length === 0 ? (
          <EmptyState tone="positive" title={t('empty')} body={t('emptyBody')} />
        ) : (
