...
/**
 * **El micro-aviso adyacente** (DESIGN_SYSTEM §22.4c, R3.1) — el aviso que va **junto a la cifra**,
 * VISIBLE para un comprador vidente, con las **dos ideas obligatorias** de PROJECT §O.5:
 ...
 * `sr-only` apoyándose en la nota al pie: invisible para quien ve la pantalla, y §O.5 lo califica de
 ...
 *
 * Reglas que este componente encarna y que NO son configurables (§22.10 nº13):
 *  - **Texto real, nunca `sr-only` / `title` / tooltip.** Lo que se ve es lo que se lee.
 *  - **Sans, no mono** (§22.4c): es prosa abreviada, no una etiqueta ni un dato — y la sans es ~20%
 ...
 *  - **Piso tipográfico propio** (§22.4d): 11px en la teja, 12px en la ficha. No baja de ahí ni a
...
export function GradingMicroNotice({
  ...
  return (
    <p className={cn('text-muted', className)}>
      {text}
      {/* La llamada cierra el aviso (§22.4a): es lo que convierte el aviso abreviado en «hay más
...
/** Las dos ideas obligatorias en tinta 500 dentro del párrafo muted (§22.4c). */
const RICH = {
