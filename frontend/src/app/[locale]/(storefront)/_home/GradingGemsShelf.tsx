...
/** Cap de la vitrina (§O.3(3)); es también el `pageSize` que se le pide al servidor. */
const GEMS_PAGE_SIZE = 8;

/** `id` de la sección: destino del enlace de regreso de la nota al pie del home (§22.4a). */
export const GRADING_GEMS_ID = 'joyas-para-gradear';

/**
 * Fuente de la vitrina «Joyas para gradear» (§22.6). La comparten la vitrina y la página del home
 ...
 *
 * No es un endpoint nuevo: es `GET /catalog/cards` filtrado (contrato v1.50), así que la vitrina usa
 * el MISMO `GroupedListingSummaryDTO` y la MISMA teja que Compra — cero drift entre superficies. El
...
/**
 * `GradingGemsShelf` — vitrina del home (§22.6). Patrón `Shelf` tal cual, con las tejas de Compra.
 *
 ...
 *    §O prohíbe (y sería un segundo rojo).
 *  - **Sin «Ver todas»** mientras el contrato no exponga una vista de Compra filtrada por elegibles
 *    (§22.6 / §22.12 nº6): no se enlaza a una vista que no filtra lo que promete.
...
export function GradingGemsShelf() {
  ...
  // Carga, error o cero elegibles ⇒ NADA (§22.6). No hay estado intermedio visible.
  ...
  return (
    ...
    >
      ...
      >
        {listings.map((l, i) => (
          // Móvil: 4 tejas visibles (§22.8). `contents` deja que la teja siga siendo el ítem de la
