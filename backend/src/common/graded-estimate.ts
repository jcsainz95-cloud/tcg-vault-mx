...
export function isStaleByOrigin(
  ...
  // QUÉ FECHA se mide. Es SEGURO en el día del despliegue porque toda fila con `evidenceDate = null` cae
  // a `capturedDate` (comportamiento idéntico al de hoy). La vía MANUAL tiene `evidenceDate = null` por
  // construcción ⇒ mide contra `capturedDate`, que es EXACTAMENTE lo que el criterio 109 pide para el
  // override manual (su captura ES su evidencia).
