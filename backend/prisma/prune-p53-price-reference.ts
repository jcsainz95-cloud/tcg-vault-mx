 ...
 *
 * ⚠️ **El `source` es el del ESCRITOR DIARIO.** El barrido diario (`ingestSinglesForSet` →
 * `PricingService.persistMarketReference`) y el escritor de import/`--force`
 * (`CardProductResolverService.upsertVariantPrice`) escriben AMBOS `source='tcgcsv_singles'` con
 * `cardProductId` poblado — la MISMA clave. La acumulación diaria (~28,559 filas/día) la generaba el
 * PRIMERO (el que corre todos los días vía cron); el segundo corre solo en import. Como comparten
 * `source`/clave, este alcance cubre las filas del escritor diario sin ajuste.
 *
