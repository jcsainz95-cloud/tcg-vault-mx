...
jest.mock('bullmq', () => ({
  Queue: jest
    ...
    .mockImplementation(() => ({
      ...
      on: queueOnMock,
      // v1.68: el scheduler retira (best-effort) el repetible viejo `guest-order-sweep`.
...
// v1.68 (§4-R.4): barrido ÚNICO de reservas vencidas (bóveda e invitado); sustituye a guest-order-sweep.
...
describe('SchedulerService — con REDIS_URL programa los diarios + price-ingest 2×/día + metadata', () => {
  ...
  it('programa fx/price/snapshot + barridos + price-ingest (default) + metadata, y los enruta', async () => {
    ...
    // v1.68 (§4-R.4): el worker enruta order-reservation-sweep a su servicio, y acepta el nombre
    // viejo `guest-order-sweep` como alias un release (repetible que pudo quedar en Redis).
    await workerProcessor!({ name: 'order-reservation-sweep' });
  ...
  it('los crons de price-ingest/metadata son overridables por env (devops)', async () => {
    ...
    // v1.68: la env `GUEST_ORDER_SWEEP_CRON` se conserva por compatibilidad y gobierna el barrido único.
    expect(byName['order-reservation-sweep']).toBe('*/30 * * * *');
